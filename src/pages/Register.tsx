...
const SPIDEY_MSGS: Record<string, string> = {
  default_Account:   "Let's build your identity, hero! 🕷️",
  full_name:         "Your display name — how others will see you",
  username:          "3-30 chars, letters, numbers & underscores only",
  email:             "Use a real email — verified domains only 🕸️",
  password:          "Min 8 chars + 1 uppercase + 1 number",
  password_show:     "Password revealed! Nobody watching? 👀",
  default_Interests: "Pick 1–4 topics — your feed spins around these! 🕸️",
...
export default function Register() {
  ...
  function validateStep1() {
    ...
    else if (form.full_name.trim().length < 2) errs.full_name = 'At least 2 characters'
    ...
    // Email — strict validation here, block moving forward
    const emailErr = validateEmail(form.email)
    if (emailErr) errs.email = emailErr

    // Password
    if (form.password.length < 8)      errs.password = 'At least 8 characters'
    else if (!/[A-Z]/.test(form.password)) errs.password = 'Must have 1 uppercase letter'
    else if (!/[0-9]/.test(form.password)) errs.password = 'Must have 1 number'
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    // Pass full_name along with register payload
    register.mutate({
      ...
      <div className="hidden lg:flex flex-1 relative overflow-hidden items-center justify-center"
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <img src="/spiderchat.png" alt="" aria-hidden="true"
            style={{ width:420, height:420, opacity:0.18, filter:'sepia(0.8) saturate(2.5) hue-rotate(10deg) brightness(0.85)', objectFit:'contain' }} />
        </div>
        <div className="absolute inset-0 pointer-events-none" style={{ background: 'radial-gradient(ellipse 50% 50% at 50% 50%, rgba(212,168,67,0.06) 0%, transparent 70%)' }} />
          ...
          <div className="space-y-2.5">
            {['Semantic AI matching', 'Redis-cached speed', 'Personalized feed'].map(f => (
              <div key={f} className="flex items-center gap-2 text-sm" style={{ color: '#8080a8' }}>
                <div className="w-4 h-4 rounded-full flex items-center justify-center shrink-0" style={{ background: 'rgba(212,168,67,0.2)' }}>
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-12 overflow-y-auto relative">
        <div className="w-full max-w-sm relative z-10">
              ...
              return (
                <div key={s} className="flex items-center gap-2 flex-1">
                  <div className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-black transition-all"
                    style={{ background: done || active ? 'linear-gradient(135deg, #d4a843, #f5e07a)' : 'rgba(255,255,255,0.05)', border: done || active ? 'none' : '1px solid rgba(255,255,255,0.1)', color: done || active ? 'black' : '#50508a' }}>
                    {done ? '✓' : i + 1}
                  </div>
          ...
          <div className="flex items-center gap-3 p-3 rounded-xl mb-5"
            ...
            <p className="text-xs leading-relaxed font-medium" style={{ color: '#d4a843' }}>{spideyMsg}</p>
          </div>
          ...
          {step === 'Account' && (
            <form onSubmit={handleNext} className="space-y-4">
              ...
              {/* Full name */}
              <div>
                <label className="text-xs font-semibold mb-1.5 block" style={{ color: '#9090b0' }}>Full Name</label>
                <input value={form.full_name} onChange={e => setField('full_name', e.target.value)}
                  onFocus={() => setFocused('full_name')} onBlur={() => setFocused('')}
                  placeholder="Vikash Sharma" autoComplete="name"
              ...
              <div>
                ...
                <input value={form.email} onChange={e => setField('email', e.target.value)}
                  onFocus={() => setFocused('email')} onBlur={() => {
                    setFocused('')
                    // Validate on blur
              ...
              <button type="submit"
                ...
                <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity"
                  style={{ background: 'linear-gradient(90deg, transparent, rgba(255,255,255,0.2), transparent)', animation: 'shimmer 1.5s infinite' }} />
          ...
          {step === 'Interests' && (
            <form onSubmit={handleSubmit} className="space-y-5">
              <InterestSelector
                ...
                onChange={interests => { setForm(f => ({ ...f, interests })); setFocused('interests') }}
